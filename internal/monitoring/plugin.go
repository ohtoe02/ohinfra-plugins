...
const (
	...
	Description = "Read-only local monitoring product diagnostics for ohtools."
)
...
func NewDefinition(options Options) protocol.Definition {
	...
	manifest := protocol.Manifest{
		...
		Commands: []protocol.Command{
			diagnostic([]string{"monitoring", "status"}, "status",
				"Show local monitoring product status"),
			diagnostic([]string{"monitoring", "inventory"}, "inventory",
				"Inventory local monitoring products"),
			diagnostic([]string{"monitoring", "config"}, "config",
				"Inspect local monitoring configuration metadata"),
		},
	}
...
func execute(ctx context.Context, invocation protocol.Invocation, options Options) (protocol.Result, error) {
	...
	switch {
	case slices.Equal(invocation.CommandPath, []string{"monitoring", "status"}):
		return executeStatus(ctx, options, local)
	case slices.Equal(invocation.CommandPath, []string{"monitoring", "inventory"}):
		return executeInventory(ctx, options, local)
	case slices.Equal(invocation.CommandPath, []string{"monitoring", "config"}):
		return executeConfig(ctx, options, local)
	default:
		return protocol.Result{}, protocol.ExitError{
			Code: protocol.ExitArguments, Err: errors.New("unsupported monitoring-base command"),
		}
	}
}
